...
class Game():
    ...
    def find_route(self, currow, curcol, curscore, curpath):
        ...
        best_score = -1
        best_path = []
        if currow == finish[0] and curcol == finish[1]:
            return (best_score, best_path)
        

        self.helper(currow + 1, curcol, curscore, curpath)
        self.helper(currow - 1, curcol, curscore, curpath)
        self.helper(currow, curcol + 1, curscore, curpath)
        self.helper(currow, curcol - 1, curscore, curpath)
        
        

    def helper(self, row, col, score, path):
        if self._maze.is_move_in_maze(row, col) == True and (row, col) not in path:
            return 'Not Valid'
        if self._maze.is_wall(row, col) == False:
            return 'Is Wall'
        if row == finish[0] and col == finish[1]:
            ...
            self._maze.make_move(row, col, path)
            score += self._maze[row][col]
            return self.find_route(row, col, score, path)       
